...
try {
  ...
  $response = $facebook->get('/me?fields=id,name,email,education',$accessToken);
} catch(Facebook\Exceptions\FacebookResponseException $e) {
...
$fbData['user_profile']['education'] = $user->getProperty("education");;
if($user) {
		$fbData['loginUrl']='';
	} else {
		$permissions = permissions();
	...
	if(isset($_GET['error'])) {
		...
		$options = array(
			...
			'plugin_user_setting_name_value_pairs' => array(
				'uid' => $fbData['user_profile']['id'],
				'access_token' => $fbData['user_profile']['accessToken'],
				'education' => $fbData['user_profile']['education'],
			),
			'plugin_user_setting_name_value_pairs_operator' => 'OR',
			'limit' => 0
		);
		...
		if (!$users) {
			...
			elgg_set_plugin_user_setting('education', $fbData['user_profile']['education']);
...
function facebook_connect_update_user_avatar($user, $file_location) {
	...
	$imgContent = getimg($file_location);
